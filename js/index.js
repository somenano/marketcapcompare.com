...
function get_selected_asset()
{
    ...
    if (unsanitized_asset === null) return window.default_selected_asset;
    const sanitized_asset = unsanitized_asset.replace(/[\W_]+/g,"");
    if (sanitized_asset=='' || sanitized_asset === undefined) return window.default_selected_asset;
...
<div class="fs-1"><span>Current Price: $`+numberWithCommas(selected_asset_price.toFixed(2))+` USD</span></div>
...
<img src="`+selected_asset_logo+`" width="32px">
 ...
 <div class="p-2 text-left" style="width:300px">
  <div><span class="fs-1-5 w-100">`+name+` (`+symbol+`)</span></div>
